...
describe TopicView do
  ...
  context '.posts' do
    ...
    describe "#gaps" do
      ...
      describe 'filtered by sort_order' do
        describe 'asc order' do
          it 'should contain the right gaps' do
            topic_view = TopicView.new(topic.id, Fabricate(:admin),
              sort_order: p5.sort_order, asc: false
            )

            expect(topic_view.contains_gaps).to eq(true)
            expect(topic_view.posts).to eq([p3, p2, p1])
        ...
        describe 'desc order' do
          it 'should contain the right gaps' do
            topic_view = TopicView.new(topic.id, Fabricate(:admin),
              sort_order: p3.sort_order, asc: true
            )

            expect(topic_view.contains_gaps).to eq(true)
            expect(topic_view.posts).to eq([p5])
            expect(topic_view.gaps.before).to eq(p5.id => [p4.id])
            expect(topic_view.gaps.after).to eq(p5.id => [p6.id, p7.id])
          end
        end
      end
